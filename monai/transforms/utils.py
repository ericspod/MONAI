...
def generate_spatial_bounding_box(
    ...
    for di, ax in enumerate(itertools.combinations(reversed(range(ndim)), ndim - 1)):
        ...
        box_start[di] = int(min_d.detach().cpu().item() if isinstance(min_d, torch.Tensor) else min_d)
        box_end[di] = int(max_d.detach().cpu().item() if isinstance(max_d, torch.Tensor) else max_d)
